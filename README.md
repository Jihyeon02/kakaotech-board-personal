...
## 프로젝트에서 얻은 점
 ...
 개발자로써 어떤 고민을 해야할지 팀원없이 스스로 모든 것을 결정하는 과정에서 많은 실패가 있었습니다. 처음에는 들어본 기술 스택을 넣기 위해 체계없이 설계와 공부를 시작하다가, 다시 되돌아가는 과정을 반복하였습니다. 그러나 그러한 실패 덕분에 하나의 기능을 개발하기 위해서 무리해서 어려운 기술을 넣기보다, "지금 내 코드에서는 어떤 한계점이 존재하길래 해당 기술이 필요한지" 이러한 문제 정의와 해결이 보이는 과정을 거쳐야한다는 것을 깨달았습니다. 앞으로도 개발실력 향상을 위한 개발은 이번 깨달음을 바탕으로 이어질 예정입니다. 
